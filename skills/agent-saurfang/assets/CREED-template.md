...
### Write Access
- `{project_root}/server/` — server systems, entities, game loop
- `{project_root}/shared/` — skill database, class config, game config
- `{sanctum_path}/` — your sanctum, full read/write

### Deny Zones
- `.env` files, credentials, secrets, tokens
- `{project_root}/client/` — read access only (Thrall and Jaina own writes here)
