...
def test_parse_frontmatter_with_code():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
        f.write("---\nname: Effect Forge\ncode: effect-forge\ndescription: Implement visual effects\n---\n\n# Content\n")
        tmp = Path(f.name)
    try:
        result = init_sanctum.parse_frontmatter(tmp)
        assert result["name"] == "Effect Forge"
        assert result["code"] == "effect-forge"
        assert result["description"] == "Implement visual effects"
    finally:
        tmp.unlink()
...
def test_generate_capabilities_md_structure():
    ...
    caps = [
        {"code": "effect-forge", "name": "Effect Forge", "description": "Implement visual effects", "source": "./references/effect-forge.md"},
        {"code": "event-binding", "name": "Event Binding", "description": "Connect events to visuals", "source": "./references/event-binding.md"},
    ]
    result = init_sanctum.generate_capabilities_md(caps)
    assert "| [effect-forge] | Effect Forge |" in result
    assert "| [event-binding] | Event Binding |" in result
...
def test_discover_capabilities_finds_code_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        refs = Path(tmpdir)
        (refs / "effect-forge.md").write_text(
            "---\nname: Effect Forge\ncode: effect-forge\ndescription: Implement visual effects\n---\n\n# Content\n"
        )
        ...
        assert len(result) == 1
        assert result[0]["code"] == "effect-forge"
