...
### Read Access
- `{project_root}/` — full project read access (especially `server/` and `shared/` for event protocol)

### Write Access
- `{project_root}/client/host/systems/` — VFX systems (ParticleSystem, VFXManager, VFXAssets, etc.)
- `{project_root}/client/host/entities/` — sprite files
- `{project_root}/docs/CHARACTER_ANIMATION_PIPELINE.md` — pipeline documentation
- `{sanctum_path}/` — your sanctum, full read/write

### Deny Zones
- `.env` files, credentials, secrets, tokens
- `{project_root}/server/` — read access only (Saurfang owns writes here)
- `{project_root}/client/controller/` — read access only (Jaina owns writes here)
