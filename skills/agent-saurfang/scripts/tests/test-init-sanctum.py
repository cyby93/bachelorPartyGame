...
def test_parse_frontmatter_with_code():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
        f.write("---\nname: Ability Forge\ncode: ability-forge\ndescription: Implement new abilities\n---\n\n# Content\n")
        tmp = Path(f.name)
    try:
        result = init_sanctum.parse_frontmatter(tmp)
        assert result["name"] == "Ability Forge"
        assert result["code"] == "ability-forge"
        assert result["description"] == "Implement new abilities"
    finally:
        tmp.unlink()
...
def test_generate_capabilities_md_structure():
    ...
    caps = [
        {"code": "ability-forge", "name": "Ability Forge", "description": "Implement new abilities", "source": "./references/ability-forge.md"},
        {"code": "combat-audit", "name": "Combat Audit", "description": "Review combat systems", "source": "./references/combat-audit.md"},
    ]
    result = init_sanctum.generate_capabilities_md(caps)
    assert "| [ability-forge] | Ability Forge |" in result
    assert "| [combat-audit] | Combat Audit |" in result
...
def test_discover_capabilities_finds_code_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        refs = Path(tmpdir)
        (refs / "ability-forge.md").write_text(
            "---\nname: Ability Forge\ncode: ability-forge\ndescription: Implement new abilities\n---\n\n# Content\n"
        )
        ...
        assert len(result) == 1
        assert result[0]["code"] == "ability-forge"
