...
# ///
"""
First Breath — Deterministic sanctum scaffolding for agent-voljin.
"""
...
def generate_capabilities_md(capabilities: list[dict]) -> str:
    ...
    lines.extend([
        ...
        "### User-Provided Tools",
        "",
        "_MCP servers, APIs, or services Cyby has made available. Document them here._",
    ])
    return "\n".join(lines) + "\n"
...
def main():
    parser = argparse.ArgumentParser(description="First Breath — scaffold the agent-voljin sanctum before conversational awakening.")
    parser.add_argument("project_root", help="Root of the project (where _bmad/ lives)")
    parser.add_argument("skill_path", help="Path to the skill directory (where SKILL.md, references/, assets/ live)")
    ...
    (sanctum_path / "CAPABILITIES.md").write_text(generate_capabilities_md(capabilities), encoding="utf-8")
    ...
    result = {
        ...
        "message": "First Breath scaffolding complete. The conversational awakening can now begin.",
    }
