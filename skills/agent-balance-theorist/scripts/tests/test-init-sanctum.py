...
# Add parent directory to path so we can import from init-sanctum
sys.path.insert(0, str(Path(__file__).parent.parent))

# Import functions from init-sanctum (rename to avoid hyphen issue)
...
def test_parse_frontmatter_with_code():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
        f.write("---\nname: Statistical Encounter Simulation\ncode: SS\ndescription: Models an encounter\n---\n\n# Content\n")
        tmp = Path(f.name)
    try:
        result = init_sanctum.parse_frontmatter(tmp)
        assert result["name"] == "Statistical Encounter Simulation"
        assert result["code"] == "SS"
        assert result["description"] == "Models an encounter"
    finally:
        tmp.unlink()
...
def test_substitute_vars():
    ...
    result = init_sanctum.substitute_vars(content, {"user_name": "Cyby", "birth_date": "2026-04-16"})
    assert result == "Hello Cyby, born on 2026-04-16."
...
def test_generate_capabilities_md_structure():
    ...
    caps = [
        {"code": "SS", "name": "Statistical Simulation", "description": "Models encounters", "source": "./references/statistical-simulation.md"},
        {"code": "LB", "name": "Level Balance Review", "description": "Holistic check", "source": "./references/level-balance-review.md"},
    ]
    result = init_sanctum.generate_capabilities_md(caps, evolvable=False)
    assert "| [SS] | Statistical Simulation |" in result
    assert "| [LB] | Level Balance Review |" in result
...
def test_discover_capabilities_finds_code_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (refs / "statistical-simulation.md").write_text(
            "---\nname: Statistical Encounter Simulation\ncode: SS\ndescription: Models an encounter\n---\n\n# Content\n"
        )
        # Write a non-capability file (no code field)
...
def test_discover_capabilities_skips_skill_only_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = init_sanctum.discover_capabilities(refs, "./references")
        # first-breath.md is in SKILL_ONLY_FILES, so should be skipped
        assert len(result) == 0
